...
RNN training for teddy Effective Resistance baseline.
Reads sequences produced by process_teddy.py from GraphPulseResultsteddy/.

No command-line arguments needed — just set DATASET below and run.

File lives at: TemporalRicci/src/teddy/rnn_teddy.py
"""
...
GRAPHPULSE_teddy_DIR = str(_BASE_DIR / "GraphPulseResultsteddy")

RESULTS_DIR  = str(_BASE_DIR / "GraphPulseResultsteddy" / "results")
RESULTS_FILE = os.path.join(RESULTS_DIR, f"RNNResults_teddy_{DATASET}.csv")
RUNTIME_FILE = os.path.join(RESULTS_DIR, f"RNN_Runtime_teddy_{DATASET}.csv")
...
def assemble_data(task_key, network, base_network, normalizer="all"):
    ...
    seq_dir  = os.path.join(GRAPHPULSE_teddy_DIR, tinfo["folder"], network)
    base_dir = os.path.join(GRAPHPULSE_teddy_DIR, tinfo["folder"], base_network)
...
if __name__ == "__main__":
    ...
    print(f"Sequences: {GRAPHPULSE_teddy_DIR}")
