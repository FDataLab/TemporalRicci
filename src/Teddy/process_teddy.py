...
but reads sparsified graphs from the flat teddy_sparcified/ folder
...
# CONFIG
# ============================================================
_THIS_DIR = Path(__file__).resolve().parent          # src/teddy/
_BASE_DIR = _THIS_DIR.parent.parent                  # TemporalRicci/


DATASET = "tgbl-comment"

teddy_DIR  = _BASE_DIR / "teddy_sparsified"
...
GRAPHPULSE_RESULTS_DIR = os.path.join(base_dir, "GraphPulseResultsteddy")
...
if not teddy_DIR.exists():
    print(f"[FATAL] teddy output folder not found: {teddy_DIR}")
    raise SystemExit(1)

teddy_csv = teddy_DIR / f"{DATASET}_teddy.csv"
if not teddy_csv.exists():
    print(f"[FATAL] File not found: {teddy_csv}")
    print(f"        Run teddy_sparsifier.py first for dataset: {DATASET}")
    raise SystemExit(1)

print(f"Dataset  : {DATASET}")
print(f"teddy CSV  : {teddy_csv}")
...
def create_rnn_sequence(
    teddy_csv_path: str,       # path to the teddy sparsified CSV
    basis_csv_path: str,     # path to the full-graph CSV (for labels)
    variant_name: str,       # e.g. "ADX_teddy"
...
timings_path   = os.path.join(GRAPHPULSE_RESULTS_DIR, "run_times_teddy.csv")
...
# Step 1: TDA on the teddy sparsified graph
daily_tda_cache = precompute_daily_tda(
    csv_path=str(teddy_csv),
...
s = total % 60
print(f"\n{'='*60}")
print(f"All teddy datasets completed in {h}h {m}m {s:.2f}s")
print(f"{'='*60}\n")