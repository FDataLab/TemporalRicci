...
but reads sparsified graphs from the flat sem_sparsified/ folder
produced by SEM_sparsifier.py.

Labels are always taken from the full graph (same as process_dataset.py)
so results are directly comparable across TFRC, MoG, TEDDY, and SEM.
...
Output layout (mirrors process_mog.py output):
    GraphPulseResultsSEM/
        Sequence_task1/
            ADX_sem_bfc/
                seq_tda.txt
                seq_raw.txt
...
File location: TemporalRicci/src/SEM/process_SEM.py

Reference
---------
Zhang, X., Song, D., & Tao, D. (2024). Ricci curvature-based graph
sparsification for continual graph representation learning. IEEE
Transactions on Neural Networks and Learning Systems, 35(12), 17398-17410.
https://doi.org/10.1109/TNNLS.2023.3303454
"""
...
_THIS_DIR = Path(__file__).resolve().parent          # src/SEM/
...
SEM_DIR  = _BASE_DIR / "sem_sparsified"
...
# Where RNN sequences are saved
GRAPHPULSE_RESULTS_DIR = os.path.join(base_dir, "GraphPulseResultsSEM")
...
if not SEM_DIR.exists():
    print(f"[FATAL] SEM output folder not found: {SEM_DIR}")
    raise SystemExit(1)

sem_csv = SEM_DIR / f"{DATASET}_sem_bfc.csv"
if not sem_csv.exists():
    print(f"[FATAL] File not found: {sem_csv}")
    print(f"        Run SEM_sparsifier.py first for dataset: {DATASET}")
    raise SystemExit(1)

print(f"Dataset  : {DATASET}")
print(f"SEM CSV  : {sem_csv}")
...
def precompute_daily_tda(csv_path: str, variant_name: str,
    ...
    for current_date in tqdm(all_dates, desc=f"{variant_name} - TDA"):
        ...
        sub_df = df[
            (df["timestamp"] >= current_date) & (df["timestamp"] < next_day)
        ]
        ...
        mapper_graph = mapper.map(
            ...
            cover=km.Cover(
                n_cubes=kwargs["n_cube"],
                perc_overlap=kwargs["over_lap"]
            )
        )
    ...
    print(
        f"[DONE] TDA {variant_name}: {blank_day_counter} empty days, "
        f"{duration:.2f}s\n"
    )
...
def create_rnn_sequence(
    sem_csv_path: str,       # path to the SEM sparsified CSV
    basis_csv_path: str,     # path to the full-graph CSV (for labels)
    variant_name: str,       # e.g. "ADX_sem_bfc"
    ...
    df       = pd.read_csv(sem_csv_path)
    df_basis = pd.read_csv(basis_csv_path)

    df["timestamp"]       = pd.to_datetime(
        df["timestamp"],       unit="s"
    ).dt.floor("D")
    df_basis["timestamp"] = pd.to_datetime(
        df_basis["timestamp"], unit="s"
    ).dt.floor("D")
    ...
    num_windows = max(0, int(
        (last_date - start_date).days - (windowSize + gap + labelWindowSize)
    ))
    ...
    for task_id, task_name, task_folder in [
        ...
        for _ in tqdm(range(num_windows),
                      desc=f"{variant_name} - {task_name}", leave=False):
            ...
            df_win   = df[
                (df["timestamp"] >= ws_date) & (df["timestamp"] < w_end)
            ]
            df_label = df[
                (df["timestamp"] >= l_start) & (df["timestamp"] < l_end)
            ]

            G = nx.from_pandas_edgelist(
                df_win, "from", "to", ["value"],
            ...
            for i in range(windowSize):
                ...
                wdeg = {
                    node: sum(
                        d.get("value", 0)
                        for _, _, d in G_day.edges(node, data=True)
                    )
                ...
                if total_wdeg > 0 and len(wdeg) > 0:
                    ...
                    whale_dominance = (
                        sum(sorted(wdeg.values(), reverse=True)[:k_top])
                        / total_wdeg
                    )
                else:
                    whale_dominance = 0
                ...
                total_volume    = sum(
                    d.get("value", 0) for _, _, d in G_day.edges(data=True)
                )
        ...
        output_dir = os.path.join(
            GRAPHPULSE_RESULTS_DIR, task_folder, variant_name
        )
        os.makedirs(output_dir, exist_ok=True)

        with open(
            os.path.join(output_dir, "seq_tda.txt"), "w", encoding="utf-8"
        ) as f:
            json.dump(
                {
                    "TDA_SEQUENCE": to_builtin(merge_dicts(seq_tda)),
                    "LABELS": seq_labels,
                },
                f, indent=2
            )
        with open(
            os.path.join(output_dir, "seq_raw.txt"), "w", encoding="utf-8"
        ) as f:
            json.dump(
                {
                    "RAW_SEQUENCE": to_builtin(merge_dicts(seq_raw)),
                    "LABELS": seq_labels,
                },
                f, indent=2
            )
    ...
    print(
        f"[DONE] RNN sequences saved for {variant_name} in {duration:.2f}s\n"
    )
...
timings_path   = os.path.join(GRAPHPULSE_RESULTS_DIR, "run_times_sem.csv")
...
variant_name = f"{DATASET}_sem_bfc"
...
# Step 1: TDA on the SEM sparsified graph
daily_tda_cache = precompute_daily_tda(
    csv_path=str(sem_csv),
...
s = total % 60
print(f"\n{'='*60}")
print(f"SEM processing completed in {h}h {m}m {s:.2f}s")
print(f"{'='*60}\n")