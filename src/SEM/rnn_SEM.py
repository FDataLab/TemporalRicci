...
RNN training for the SEM Balanced Forman Curvature baseline.
Reads sequences produced by process_SEM.py from GraphPulseResultsSEM/.

No command-line arguments needed — just set DATASET below and run.

File lives at: TemporalRicci/src/SEM/rnn_SEM.py
...
# Sequences produced by process_SEM.py
GRAPHPULSE_SEM_DIR = str(_BASE_DIR / "GraphPulseResultsSEM")

# Results output
RESULTS_DIR  = str(_BASE_DIR / "GraphPulseResultsSEM" / "results")
RESULTS_FILE = os.path.join(RESULTS_DIR, f"RNNResults_SEM_{DATASET}.csv")
RUNTIME_FILE = os.path.join(RESULTS_DIR, f"RNN_Runtime_SEM_{DATASET}.csv")
...
def assemble_data(task_key, network, base_network, normalizer="all"):
    ...
    seq_dir  = os.path.join(GRAPHPULSE_SEM_DIR, tinfo["folder"], network)
    base_dir = os.path.join(GRAPHPULSE_SEM_DIR, tinfo["folder"], base_network)
...
def train_and_log(task_key, network, X, y, diag):
    ...
    with open(RUNTIME_FILE, "a") as f:
        f.write(
            f"{task_key},{network},{start_datetime},{end_datetime},{total_seconds:.2f}\n"
        )
...
if __name__ == "__main__":
    ...
    print(f"Sequences: {GRAPHPULSE_SEM_DIR}")
